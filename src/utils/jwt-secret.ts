...
export async function getOrGenerateJWTSecret(
    ...
    if (env.JWT_SECRET && isValidJWTSecret(env.JWT_SECRET)) {
        console.log('✅ 使用环境变量中的 JWT Secret');
    ...
    if (db) {
        try {
            ...
            if (result && isValidJWTSecret(result.value as string)) {
                console.log('✅ 使用数据库中的 JWT Secret');
    ...
    if (db) {
        try {
            ...
            `).bind(newSecret).run();
            console.log('✅ 新的 JWT Secret 已保存到数据库');
        } catch (error) {
    ...
    if (db) {
        try {
            ...
            if (oldResult && oldResult.value) {
                ...
                console.log('📌 包含旧 JWT Secret 用于验证');
            }
        } catch (error) {
        ...
        if (result && result.description) {
            ...
            if (expiryMatch) {
                ...
                if (new Date() > expiryDate) {
                    ...
                    console.log('🧹 已清理过期的旧 JWT Secret');
                }
            }
        }
    } catch (error) {
