...
export async function findMailboxesByUserId(db: D1Database, userId: number): Promise<Mailbox[]> {
    ...
    return result.results as Mailbox[];
}
...
export async function createMailbox(
    ...
    const result = await db.prepare(`
        INSERT INTO mailboxes (user_id, email_address, is_active, created_at, updated_at)
...
export async function getAllMailboxes(
    ...
    page: number = 1, 
    pageSize: number = 20
): Promise<{ mailboxes: (Mailbox & { user_email_prefix: string; user_type: string })[], total: number }> {
    ...
    const result = await db.prepare(`
        SELECT 
            ...
            m.created_at,
            m.updated_at,
            u.email_prefix as user_email_prefix,
            u.user_type
        FROM mailboxes m
        JOIN users u ON m.user_id = u.id
        WHERE m.is_active = 1
    ...
    return {
        mailboxes: result.results as (Mailbox & { user_email_prefix: string; user_type: string })[],
        total
    };
}
...
export async function createMailboxApplication(
    ...
    return application as MailboxApplication;
}
...
export async function getUserMailboxApplications(db: D1Database, userId: number): Promise<MailboxApplication[]> {
    ...
    return result.results as MailboxApplication[];
}
...
export async function getAllMailboxApplications(
    ...
    page: number = 1,
    pageSize: number = 20
): Promise<{ applications: (MailboxApplication & { user_email_prefix: string })[], total: number }> {
    const offset = (page - 1) * pageSize;
    ...
    return {
        applications: result.results as (MailboxApplication & { user_email_prefix: string })[],
        total
    };
}
