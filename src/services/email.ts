...
export async function createEmail(
    ...
    const result = await db.prepare(`
        INSERT INTO emails (
            ...
            text_content, html_content, raw_email, has_attachments, received_at,
    ...
    `).bind(
        ...
        emailData.text_content || null,
        emailData.html_content || null,
...
export async function getEmailById(db: D1Database, id: number): Promise<Email | null> {
    ...
    return {
        ...
        text_content: result.text_content as string | undefined,
        html_content: result.html_content as string | undefined,
...
export async function getUserEmails(
    ...
    // 构建查询条件
    if (search) {
        conditions.push('(subject LIKE ? OR text_content LIKE ? OR sender_email LIKE ?)');
    ...
    const emailsResult = await db.prepare(`
        SELECT id, message_id, user_id, sender_email, recipient_email, subject,
               text_content, html_content, has_attachments, received_at,
               created_at, updated_at
        FROM emails
        WHERE ${whereClause}
        ${orderClause}
...
export async function getAllEmails(
    ...
    // 构建查询条件
    if (search) {
        conditions.push('(subject LIKE ? OR text_content LIKE ? OR sender_email LIKE ? OR recipient_email LIKE ?)');
        const searchPattern = `%${search}%`;
