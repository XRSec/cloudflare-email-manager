...
function insertLargeEmail(messageId, userId, from, to, subject, content, contentType, rawEmail, receivedAt) {
    try {
        ...
        // 执行SQL
        const command = `docker exec node bash -c 'wrangler d1 execute cem-db --file="${sqlFile}" --json'`;
...
function executeSQL(sql) {
    ...
    try {
        ...
        // 使用Docker容器执行wrangler命令，本地数据库不需要 --remote 参数
        const command = `docker exec node bash -c 'wrangler d1 execute cem-db --file="${tempFile}" --json'`;
