...
export const EmailTemplates = {
     ...
     * 站内邮件发送表单
     */
    internalEmailForm: createTemplate(`
        <div class="internal-email-form">
            <h4>发送站内邮件</h4>
            <form id="{{formId}}" method="post" action="javascript:void(0);" onsubmit="return false;">
                <div class="form-group">
                    <label for="intFrom">发件人:</label>
                    <input type="email" id="intFrom" name="from" required class="form-control" placeholder="{{fromEmail}}" value="{{fromEmail}}">
                </div>
                <div class="form-group">
                    <label for="intTo">收件人:</label>
                    <input type="email" id="intTo" name="to" required class="form-control" placeholder="{{toEmail}}" value="{{toEmail}}">
                </div>
                <div class="form-group">
                    <label for="intSubject">主题:</label>
                    <input type="text" id="intSubject" name="subject" required class="form-control" placeholder="{{subject}}" value="{{subject}}">
                </div>
                <div class="form-group">
                    <label for="intContent">邮件内容 (支持 Markdown):</label>
                    <div class="markdown-editor">
                        <div class="editor-tabs">
                            <button type="button" class="tab-btn active" onclick="EmailManager.switchEditorTab('write')">编写</button>
                            <button type="button" class="tab-btn" onclick="EmailManager.switchEditorTab('preview')">预览</button>
                        </div>
                        <div class="editor-content">
                            <textarea id="intContent" name="content" class="form-control markdown-textarea" rows="8" placeholder="支持 Markdown 语法，例如：&#10;&#10;# 标题&#10;**粗体** *斜体*&#10;&#10;- 列表项 1&#10;- 列表项 2&#10;&#10;代码块示例：&#10;function hello() {&#10;  return 'world';&#10;}&#10;&#10;[链接](https://example.com)" >{{content}}</textarea>
                            <div id="markdownPreview" class="markdown-preview" style="display: none;"></div>
                        </div>
                    </div>
                    <div class="markdown-help">
                        <small>支持 Markdown 语法：标题 (# ## ###)、粗体 (**text**)、斜体 (*text*)、列表 (- item)、代码块、链接 ([text](url))</small>
                    </div>
                </div>
                <div class="form-actions">
                    <button type="button" class="btn btn-primary" onclick="EmailManager.sendInternalEmail()">发送站内邮件</button>
                    <button type="button" class="btn btn-secondary" onclick="EmailManager.clearInternalForm()">清空表单</button>
                </div>
            </form>
        </div>
    `)
        ...
        <div class="debug-panel">
            ...
            <div class="debug-info">
                <div class="debug-item">
                    <label>系统状态:</label>
                    <span id="debugSystemStatus" class="status-loading">检测中...</span>
                </div>
                <div class="debug-item">
                    <label>当前用户:</label>
                    <span id="debugCurrentUser">加载中...</span>
                </div>
                <div class="debug-item">
                    <label>调试模式:</label>
                    <span id="debugModeStatus" class="status-loading">检测中...</span>
                </div>
                <div class="debug-item">
                    <label>系统配置:</label>
                    <span id="debugSystemConfig">加载中...</span>
                </div>
                <div class="debug-item">
                    <label>最后更新:</label>
                    <span id="debugLastUpdate">刚刚</span>
                </div>
            </div>
            <div class="debug-actions">
                <button class="btn btn-primary" onclick="DebugManager.refreshDebugInfo()">刷新调试信息</button>
                <button class="btn btn-secondary" onclick="DebugManager.simulateEmailReceive()">模拟邮件</button>
                <button class="btn btn-warning" onclick="DebugManager.clearDebugLogs()">清空日志</button>
            </div>
        </div>
    `),
    ...
    simulateEmailForm: createTemplate(`
        <div class="simulate-form">
            ...
            <form id="{{formId}}" method="post" action="javascript:void(0);" onsubmit="return false;">
                ...
                <div class="form-group">
                    <label for="simContentType">内容类型:</label>
                    <select id="simContentType" name="content_type" class="form-control" onchange="DebugManager.toggleContentType()">
                        <option value="text">纯文本</option>
                        <option value="html">HTML</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="simContent">邮件内容:</label>
                    <textarea id="simContent" name="content" class="form-control" rows="6" placeholder="请输入邮件内容..." required></textarea>
                </div>
                <div class="form-actions">
                    <button type="button" class="btn btn-primary" onclick="DebugManager.simulateEmailReceive()">发送模拟邮件</button>
