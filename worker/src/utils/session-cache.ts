/**
 * SessionStorage 缓存管理系统
 * 用于在浏览器会话期间缓存系统配置、用户信息等数据
 * 数据会在标签页关闭后自动清除
 */
...
class SessionStorageCache {
    ...
    set<T>(key: string, data: T, ttl?: number): void {
        if (typeof window === 'undefined' || !window.sessionStorage) {
            console.warn('[SessionCache] SessionStorage 不可用');
            return;
        }
        ...
        try {
            ...
            if (error instanceof DOMException && error.name === 'QuotaExceededError') {
                ...
                try {
                    ...
                    console.error(`[SessionCache] 重试设置缓存失败: ${key}`, retryError);
                }
            }
        }
    }
    ...
    get<T>(key: string): T | null {
        ...
        try {
            ...
            if (Date.now() - item.timestamp > item.ttl) {
                this.delete(key);
                console.log(`[SessionCache] 缓存已过期并删除: ${key}`);
                return null;
            }
    ...
    delete(key: string): void {
        ...
        console.log(`[SessionCache] 缓存已删除: ${key}`);
    }
    ...
    clear(): void {
        ...
        console.log(`[SessionCache] 已清空 ${keysToRemove.length} 个缓存项`);
    }
    ...
    refresh(key: string): void {
        this.delete(key);
        console.log(`[SessionCache] 缓存已刷新: ${key}`);
    }
    ...
    cleanExpired(): void {
        ...
        if (cleanedCount > 0) {
            console.log(`[SessionCache] 清理了 ${cleanedCount} 个过期缓存项`);
        }
    }
