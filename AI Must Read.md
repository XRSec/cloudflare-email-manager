...
1. 模块化 index.ts 拆分,非常重要
2. 调试模式 和 是否允许注册可以通过接口传递到 前端,这个请求只需要请求一次,设置里面的重新加载可以做成全局按钮,主要用途就是刷新当前页面的数据,可以看一下如何做耦合,我希望能用上类型 VUE 的模块互,不单独指 html, js, css 也可以,而且 CSS 可以考虑复用