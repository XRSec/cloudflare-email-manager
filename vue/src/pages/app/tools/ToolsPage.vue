<template>
  <div class="tools-page">
    <div class="page-header">
      ...
      <span class="tools-badge">请小心使用！</span>
    </div>

    <div class="tools-content">
      <!-- 系统信息 -->
      <div class="tools-section" ref="systemSectionRef">
        <h2>系统信息</h2>
        <div class="info-grid">
          <div class="info-item">
            <label>用户信息:</label>
      ...
      <!-- 数据库管理 -->
      <div class="tools-section" ref="databaseSectionRef">
        <h2>数据库管理</h2>
        <p class="tools-note">数据库信息：查看SQLite版本、记录总数等基本信息<br>
    ...
    <!-- 缓存管理 -->
    <div class="tools-section" ref="cacheSectionRef">
      <h2>缓存管理</h2>
      <p class="tools-note">
    ...
    <!-- R2 文件管理 -->
    <div class="tools-section" ref="r2SectionRef">
      <h2>R2 文件管理</h2>
      <p class="tools-note">查看 R2 存储中的文件列表，包括邮件附件和原始邮件文件。不获取文件内容，仅显示文件元数据信息。</p>

      <div class="r2-actions">
        <button class="btn btn-secondary" @click="loadR2Files" :disabled="r2Loading">
          {{ r2Loading ? '加载中...' : '📋 获取 R2 文件列表' }}
        </button>
        <button class="btn btn-primary" @click="restoreEmailsFromR2" :disabled="r2Restoring">
          {{ r2Restoring ? '恢复中...' : (r2RestoreCursor ? '📨 继续恢复邮件索引' : '📨 从 R2 恢复邮件索引') }}
        </button>
        <button class="btn btn-danger" @click="deleteSelectedR2Files"
          :disabled="r2Loading || r2Deleting || r2Restoring || selectedR2Files.length === 0">
      ...
      <div v-if="r2RestoreResult" class="r2-restore-result">
        <h3>恢复结果</h3>
        <div class="restore-stats">
          <span>扫描 {{ r2RestoreResult.scanned || 0 }} 封</span>
          <span>新增 {{ r2RestoreResult.restored || 0 }} 封</span>
          <span>跳过 {{ r2RestoreResult.skipped || 0 }} 封</span>
          <span>附件 {{ r2RestoreResult.attachmentsRestored || 0 }} 个</span>
          <span>失败 {{ r2RestoreResult.failed || 0 }} 封</span>
        </div>
        <p v-if="r2RestoreResult.hasMore" class="tools-note">R2 中还有更多邮件文件，可再次点击继续恢复。</p>
        <ul v-if="r2RestoreResult.errors?.length" class="restore-errors">
          <li v-for="error in r2RestoreResult.errors.slice(0, 5)" :key="error">{{ error }}</li>
        </ul>
      </div>
    ...
    <!-- KV 缓存管理 -->
    <div class="tools-section" ref="kvSectionRef">
      <h2>KV 缓存管理</h2>
      <p class="tools-note">
...
const r2SectionRef = ref<HTMLElement | null>(null)
const kvSectionRef = ref<HTMLElement | null>(null)

// 根据路由 query.section 滚动到对应区域
const scrollToSection = (section?: string | null) => {
  ...
  switch (section) {
    ...
    case 'api':
      ...
      break
    case 'database':
      target = databaseSectionRef.value
      break
    case 'cache':
      target = cacheSectionRef.value
      break
    case 'r2':
      target = r2SectionRef.value
      break
    case 'kv':
      target = kvSectionRef.value
      break
    case 'forward-logs':
      // 目前转发日志属于仪表盘 / 其他页面，这里预留占位，避免无效 section 报错
      break
    default:
      break
  }
...
const r2Restoring = ref(false)
const r2RestoreResult = ref<any>(null)
const r2RestoreCursor = ref<string | null>(null)
...
const showClearCacheDialog = async () => {
  try {
    const ElMessageBox = await getMessageBox()
...
const loadDatabaseInfo = async () => {
  ...
  try {
    const response = await fetch('/api/tools/d1/info', {
      method: 'GET',
      credentials: 'include',
      headers: {
...
const loadAllTablesData = async () => {
  ...
  try {
    const response = await fetch('/api/tools/d1/tables', {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
...
const initializeDatabase = async () => {
  ...
  try {
    const response = await fetch('/api/tools/d1/init', {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
...
const loadDatabaseStats = async () => {
  ...
  try {
    const response = await fetch('/api/tools/d1/stats', {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
...
const testDatabaseConnection = async () => {
  ...
  try {
    const response = await toolsApiService.getDatabaseInfo()
...
const loadR2Files = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/tools/r2?${params.toString()}`, {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
...
const restoreEmailsFromR2 = async () => {
  if (!confirm('确定要从 R2 的 email:*.eml 文件恢复邮件索引吗？\n\n此操作会向 D1 写入缺失的邮件记录，已存在的邮件会跳过。')) {
    return
  }

  r2Restoring.value = true
  r2RestoreResult.value = null
  try {
    const response = await fetch('/api/tools/r2/restore-emails', {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
      ...
      body: JSON.stringify({
        ...
        cursor: r2RestoreCursor.value || undefined
      })
    })
    ...
    if (result.success) {
      r2RestoreResult.value = result.data
      r2RestoreCursor.value = result.data?.hasMore ? result.data.cursor : null
      toast.success(result.message || 'R2 邮件索引恢复完成')
    } else {
      throw new Error(result.message || 'R2 邮件索引恢复失败')
    }
  } catch (error) {
    console.error('R2 邮件索引恢复失败:', error)
    toast.error('R2 邮件索引恢复失败: ' + (error as Error).message)
  } finally {
    r2Restoring.value = false
  }
}
...
const deleteR2Files = async (keys: string[]) => {
  ...
  try {
    const response = await fetch('/api/tools/r2', {
      method: 'DELETE',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
...
const loadKvCacheList = async () => {
  ...
  try {
    const response = await fetch('/api/tools/kv/list', {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
...
const deleteKvCacheByKeyInternal = async (key: string) => {
  ...
  try {
    ...
    const response = await fetch(`/api/tools/kv/delete/${encodedKey}`, {
      method: 'DELETE',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json'
...
onMounted(() => {
  // 根据 URL 中的 section 参数滚动到对应的工具区块
  nextTick(() => {
    const section = (route.query.section as string | undefined) || null
    if (section) {
...
// 监听路由 query.section 变化（例如从其它页面多次跳转到不同 section）
watch(
  () => route.query.section,
  (newVal, oldVal) => {
    if (newVal && newVal !== oldVal) {
      nextTick(() => {
        scrollToSection(String(newVal))
      })
    }
  }
)
...
.tools-badge {
  padding: 8px 12px;
  border-radius: 999px;
  background: linear-gradient(135deg, #f05f57 0%, #e63f4d 100%);
  box-shadow: 0 14px 24px -18px rgba(224, 64, 71, 0.85);
}
...
.tools-note {
  border-radius: 18px;
  padding: 14px 16px;
  background: linear-gradient(180deg, rgba(234, 245, 255, 0.95), rgba(245, 249, 255, 0.95));
  border-color: rgba(80, 139, 205, 0.18);
  line-height: 1.6;
}
...
.info-item {
  ...
  border-radius: 18px;
  border: 1px solid rgba(15, 23, 42, 0.08);
  box-shadow: 0 18px 30px -34px rgba(15, 23, 42, 0.65);
}
...
.tools-page .form-control {
  ...
  border-color: rgba(52, 84, 117, 0.18);
}
