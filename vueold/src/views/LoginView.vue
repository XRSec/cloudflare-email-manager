  ...
  <div class="login-page">
    <div id="loginSection" class="card">
      ...
      <div v-if="activeTab === 'login'" class="tab-content active">
        <!-- 加载动画 -->
        <LoadingOverlay v-if="loading" :show="true" text="登录中..." type="inline" size="medium" :overlay="false" />
      ...
      <div v-if="activeTab === 'register'" class="tab-content active">
        <AppLoadingSpinner v-if="loading" :show="true" text="注册中..." type="inline" size="medium" :overlay="false" />
...
const handleLoginSuccess = () => {
  // TODO 我想做懒加载 还有 redirect 我想要的是登录页面隐藏显示后台页面
...
const loadRegistrationStatus = async () => {
  try {
    ...
    // 使用系统健康状态接口
    const health = await unifiedCache.getSystemHealth()
    if (health) {
      allowRegistration.value = health.config?.allow_registration === 1
      return
    }

    // 降级到注册状态接口
    allowRegistration.value = await unifiedCache.getRegistrationStatus()
  } catch (error) {
...
@media (max-width: 768px) {
  ...
  .form-control { padding: 12px 16px; font-size: 16px; }
